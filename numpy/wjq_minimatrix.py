...
class Matrix:
	...
	def reshape(self, newdim):
		...
		newdata=[0 for _ in range(newdim[1])]*newdim[0]
		x=0
		y=0
	...
	def sum(self, axis=None): 
		...
		if axis==None:
			...
			newdata=[[0]*self.dim[1]]
			newdim=(1,self.dim[1])
			for i in range(self.dim[0]):
				for j in range(self.dim[1]):
					newdata[1][j]+=self.data[i][j]
			return Matrix(newdata,newdim)

		elif axis==1:
			newdata=[[0]]*self.dim[0]
			newdim=(self.dim[0],1)
			for i in range(self.dim[0]):
				for j in range(self.dim[1]):
					newdata[i][1]+=self.data[i][j]
			return Matrix(newdata,newdim)
